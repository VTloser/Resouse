...
namespace Xamin
{
    ...
    public class CircleSelector : MonoBehaviour
    {
        ...
        {
            if (Input.GetMouseButtonDown(2))  //唤醒圆环
            {
                Open();
                Debug.Log("中间事件");
            }
        ...
        public void Open()
        {
            //transform.transform.position = Input.mousePosition;
            opened = true;
            transform.localScale = (openAnimation == AnimationType.zoomIn) ? Vector3.zero : Vector3.one * 100;
        }

        public void Close_Abandon()
        {
            opened = false;
        }
        ...
        void Update()
        {
            ...
            if (opened)
            {
                ...
                if (transform.localScale.x >= size - .2f)
                {
                    ...
                    if (Input.GetMouseButtonUp(0))   //结束选择
                    {
                        if (SelectedSegment.GetComponent<Xamin.Button>().unlocked)
                        {
                    ...
                    if (Input.GetMouseButtonDown(1))  //结束未选择
                    {
                        Close_Abandon();
                    }
                    #endregion
            ...
            {
                transform.localScale = Vector3.Lerp(transform.localScale, (closeAnimation == AnimationType.zoomIn) ? Vector3.zero : Vector3.one * 3.5f, .05f);
                cursor.color = Color.Lerp(cursor.color, Color.clear, lerpAmount / 5f);
                if (this.GetComponent<CanvasGroup>().alpha > 0f)
                {

                    this.GetComponent<CanvasGroup>().alpha = Mathf.Lerp(this.GetComponent<CanvasGroup>().alpha, -0.1f, lerpAmount / 5f);
                    //Debug.Log(this.GetComponent<CanvasGroup>().alpha);

                }
                //this.GetComponent<CanvasGroup>().alpha = Mathf.Lerp(1, 0, lerpAmount / 5f);
