...
namespace YSuperMan
{
    ...
    public class MyDIs : MonoBehaviour
    {
        ...
        public FOM fadingMode = FOM.outByTransparencyColor;
            ...
            switch (fadingMode)
            {
                ...
                case FOM.outByTransparencyColor:
                    if (_componentTypes[i] == 0)
                    {
                        ...
                        //    yield return null;
                        //}
                        Group = targetObject[0].GetComponent<CanvasGroup>();
                        if (Group == null)
                            Group = targetObject[0].AddComponent<CanvasGroup>();
                        while (Group.alpha > 0f)
                        {
                            t += Time.deltaTime / fadingDuration;
                            Group.alpha = Mathf.Lerp(Group.alpha, 0f, t);
                            yield return null;
                        }
                    }
                    else if (_componentTypes[i] == 1)
